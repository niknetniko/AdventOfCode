...
public class Main {
    ...
    private static class Node {
        ...
        private Node(String parent, String name, List<String> children) {
            this.parent = parent;
            this.name = name;
    ...
    private static Pair<Integer, Integer> weightCalc(String parent, Map<String, List<String>> children, Map<String, Integer> weights) {
        if (!children.containsKey(parent)) {
            return new Pair<>(weights.get(parent), -1);
        } else {
            ...
            if (frequencies.size() > 1) {
                ...
                Map.Entry<Integer, Integer> max = Collections.min(frequencies.entrySet(), Comparator.comparingInt((ToIntFunction<Map.Entry<Integer, Integer>>) Map.Entry::getValue).reversed());
                return new Pair<>(-1, weights.get(childs.get(min.getKey())) + (max.getKey() - min.getKey()));
            }
