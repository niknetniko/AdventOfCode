...
public class Main {
    ...
    public static void one() throws FileNotFoundException {
        ...
        long result = StreamEx.of(new BufferedReader(new FileReader("Reverse Captcha - 1/input/input.txt")).lines())
                .map(s -> s + s.charAt(0))
    ...
    public static void two() throws FileNotFoundException {
        List<Integer> l = StreamEx.of(new BufferedReader(new FileReader("Reverse Captcha - 1/input/input.txt")).lines())
                .map(s -> s + s.charAt(0))
                .flatMap(s -> s.chars().boxed())
                .map(integer -> integer - '0')
