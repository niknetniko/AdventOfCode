...
public abstract class Pair<L, R> implements Map.Entry<L, R>, Serializable {
    ...
    /**
     * Get a pair as a stream. Only works if both elements are the same type and not null.
     *
     ...
     */
    public static <L> Stream<L> stream(Pair<L, L> pair) {
        return StreamSupport.stream(iterable(pair).spliterator(), false);
    }

    /**
     * Create an iterable from the pair. Only works if both elements are the same type and not null.
     *
     * @param pair The pair to convert.
     * @param <L>  The type of the elements.
     * @return An iterable for the pair.
     */
    public static <L> Iterable<L> iterable(Pair<L, L> pair) {
        return () -> new Iterator<>() {
            ...
            public L next() {
                ...
                throw new NoSuchElementException("No more elements in the pair.");
            }
        };
    }
}