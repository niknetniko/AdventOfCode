package be.strijbol.advent.mmxvii.day3;
...
public class Main {
    ...
    public static long getSquare(long number) {
        ...
        Pair<Long, Long> startCoordinate = Pair.of(n, -n + 1);
        ...
        while(currentNumber < number) {
            if (passed < (numbersOnSquare / 4)) {
                coordinate = Pair.of(coordinate.getKey(), coordinate.getValue() + 1);
            } else if (passed < 2*(numbersOnSquare / 4)) {
                coordinate = Pair.of(coordinate.getKey() - 1, coordinate.getValue());
