! FROM https://fortran-lang.github.io/fpm/src/fpm_strings.f90
...
contains
    ...
    subroutine split(input_line, array, delimiters, order, nulls)
        ...
        if(present(order))then; ordr = lower(adjustl(order));
        else; ordr = 'sequential';
        endif ! decide on value for optional ORDER parameter
        if(present(nulls))then; nlls = lower(adjustl(nulls));
        else; nlls = 'ignore'    ;
        endif ! optional parameter
    ...
    pure function join(str, sep, trm, left, right, start, end) result (string)
        ...
        logical :: trm_local
        integer :: i
        if(present(sep))then   ; sep_local = sep     ;
        else ; sep_local = ''     ;
        endif
        if(present(trm))then   ; trm_local = trm     ;
        else ; trm_local = .true. ;
        endif
        if(present(left))then  ; left_local = left   ;
        else ; left_local = ''    ;
        endif
        if(present(right))then ; right_local = right ;
        else ; right_local = ''   ;
        endif
        string = ''
        if(size(str)==0)then
    ...
    type(string_t) function module_prefix_type(custom_prefix) result(ptype)
        type(string_t), intent(in) :: custom_prefix
